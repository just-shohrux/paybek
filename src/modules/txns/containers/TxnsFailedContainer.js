        ...
        <Container fluid>
            <Row>
                ...
                <Col xs={12} className={'mb-8'}>
                    <BaseBreadcrumb items={[{id: 1, name: 'TXNS', url: '/txns'},{id: 2, name: 'Failed', url: '/txns/failed'}]}/>
                </Col>
                <Col xs={12}>
                    <Content>
                        <Row align={'center'} className={'mb-16'}>
                            <Col xs={8}>
                                <Flex>
                                    <BaseDatePicker value={get(filter,'fromDate')} defaultDate={moment(get(filter,'fromDate')).toDate()}
                                                    handleDate={(date) => setFilter(filter => ({
                                                        ...filter,
                                                        fromDate: moment(date).format('YYYY-MM-DD')
                                                    }))} placeholder={'From'} classNAme={'mr-8'}/>
                                    <BaseDatePicker value={get(filter,'toDate')} defaultDate={moment().add(1, 'days').toDate()}
                                                    handleDate={(date) => setFilter(filter => ({
                                                        ...filter,
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(TxnsFailedContainer));
