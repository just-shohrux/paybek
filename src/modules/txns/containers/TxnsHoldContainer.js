...
const TxnsHoldContainer = ({
    ...
    const [filter, setFilter] = useState({  txnStatusEnum: "HOLD",countryId: "", fromDate:moment('2021-09-01').format("YYYY-MM-DD"),toDate:moment().add(1,'days').format("YYYY-MM-DD"),searchingField: "", size: pageSize, page: pageNumber});
        ...
        <Container fluid>
            <Row>
                ...
                <Col xs={12} className={'mb-8'}>
                    <BaseBreadcrumb items={[{id: 1, name: 'TXNS', url: '/txns'},{id: 2, name: 'On Hold', url: '/txns/hold'}]}/>
                </Col>
                <Col xs={12}>
                    <Content>
                        <Row align={'center'} className={'mb-16'}>
                            <Col xs={8}>
                                <Flex>
                                    <BaseDatePicker value={get(filter,'fromDate')} defaultDate={moment(get(filter,'fromDate')).toDate()} handleDate={(date) =>setFilter(filter => ({...filter,fromDate: moment(date).format('YYYY-MM-DD')}))} placeholder={'From'} classNAme={'mr-8'}/>
                                    <BaseDatePicker value={get(filter,'toDate')} defaultDate={moment().add(1,'days').toDate()} handleDate={(date) =>setFilter(filter => ({...filter,toDate: moment(date).format('YYYY-MM-DD')}))} placeholder={'Till'} classNAme={'mr-8'} margin={'0 0 0 15px'}/>
                                    <BaseSelect ref={refCountrySelect} handleChange={(values) => setFilter(filter => ({
                                        ...filter,
                                        countryId: get(values, 'value', '')
                                    }))} options={countries} placeholder={'Filter by country'} width={'200px'} margin={'0 0 0 15px'}/>
                                    <BaseInput
                                        value={clear}
                            ...
                            <Col xs={4} className={'text-right'}>
                                <BaseButton handleClick={() => {
                                    setFilter(filter => ({...filter,fromDate:moment('2021-09-01').format("YYYY-MM-DD"),toDate:moment().add(1,'days').format("YYYY-MM-DD"), roleId: "", countryId: ""}));
                                    clearSelect()
                                ...
                                {isFetched ?
                                    <BaseTable
                                        ...
                                        {!isEmpty(txns) ? txns.map((item, index) => <tr key={get(item, 'id', null)}>
                                            ...
                                            <td>{moment(get(item, 'date')).format('DD.MM.YYYY A')}</td>
                                            <td><ReactSVG src={holdIcon} /></td>
                                        </tr>) : <tr>
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(TxnsHoldContainer));
